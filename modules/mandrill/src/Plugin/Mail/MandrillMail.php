...
class MandrillMail implements MailInterface {
  ...
  public function mail(array $message) {
    ...
    //if the header reply to is not set or empty addd the FROM address to as a Reply-to
    if (!empty($message['from']) && (!isset($header_key_map['reply-to']) || empty($headers[$header_key_map['reply-to']]))) {
      $reply_to_key = isset($header_key_map['reply-to']) ? $header_key_map['reply-to'] : 'Reply-to';
      $headers[$reply_to_key] = $message['from'];
    }
    // Prepare attachments.
    $attachments = array();
